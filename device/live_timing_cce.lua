...
function device.OnInit(params, node)
	...
	menuSend:AppendSeparator();
	btn_saisie_Inter = menuSend:Append({label = "Saisie des infos pour points Intermédiaires", image = "./res/32x32_cloud_blue.png"});
	btn_envoi_Inter = menuSend:Append({label = "Envoi des infos points Intermédiaires", image = "./res/32x32_cloud_blue.png"});
	...
	panel:Bind(eventType.MENU, OnSendInter, btn_envoi_Inter);
...
-- Grid Options
	...
	Table_Epreuve_Passage:SetColumn('Distance', { label = 'Distance', width = 9 });
	...
	grid:Set({
		...
		columns = 'Code, Code_epreuve, Code_manche, Code_Passage, Distance, Num_Portes, Libelle, Altitude, Lieu, Info ',
...
function OnSendInter(evt)
	...
	for i=0, Grid_Ligne:GetNbRows()-1 do
		--code_evenement = tonumber(live.Code_evenement);
...
function CommandRaceInfo(activerun, bolPlusStartList)
	...
	--Envoi des infos des Passages
	-- for i=0, tEpreuvePassage:GetNbRows()-1 do
		-- local nodeEpreuve = xmlNode.Create(nodeRaceinfo, xmlType.ELEMENT_NODE, "FFS_EpreuvePassage");
				
		-- xmlNode.Create(nodeEpreuve, xmlType.ELEMENT_NODE, "FFS_Code_evenement", tEpreuvePassage:GetCellInt('Code_evenement', i));	
		-- xmlNode.Create(nodeEpreuve, xmlType.ELEMENT_NODE, "FFS_Code_epreuve", tEpreuvePassage:GetCellInt('Code_epreuve', i));	
		-- xmlNode.Create(nodeEpreuve, xmlType.ELEMENT_NODE, "FFS_Code_manche", tEpreuvePassage:GetCellInt('Code_manche', i));	
		-- xmlNode.Create(nodeEpreuve, xmlType.ELEMENT_NODE, "FFS_Code_Passage", tEpreuvePassage:GetCellInt('Code_Passage', i));
		-- xmlNode.Create(nodeEpreuve, xmlType.ELEMENT_NODE, "FFS_Distance", tEpreuvePassage:GetCellInt('Distance', i));	
		-- xmlNode.Create(nodeEpreuve, xmlType.ELEMENT_NODE, "FFS_Distance_Cumulee", tEpreuvePassage:GetCellInt('Distance_Cumulee', i));	
		-- xmlNode.Create(nodeEpreuve, xmlType.ELEMENT_NODE, "FFS_Libelle", tEpreuvePassage:GetCell('Libelle', i));	
		-- xmlNode.Create(nodeEpreuve, xmlType.ELEMENT_NODE, "FFS_Altitude", tEpreuvePassage:GetCellInt('Altitude', i));	
		-- xmlNode.Create(nodeEpreuve, xmlType.ELEMENT_NODE, "FFS_Speed", tEpreuvePassage:GetCellInt('Speed', i, '%4Y-%2M-%2D'));	
		-- xmlNode.Create(nodeEpreuve, xmlType.ELEMENT_NODE, "FFS_Lieu", tEpreuvePassage:GetCell('Lieu', i));
		-- xmlNode.Create(nodeEpreuve, xmlType.ELEMENT_NODE, "FFS_Reserve", tEpreuvePassage:GetCellInt('Reserve', 0));
		-- xmlNode.Create(nodeEpreuve, xmlType.ELEMENT_NODE, "FFS_Info", tEpreuvePassage:GetCell('Info', i));
	-- end
