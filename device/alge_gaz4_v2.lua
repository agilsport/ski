...
function OnTimer(evt)
	...
	elseif state == stateBib.FINISHED then
		...
		if tickCount - tickcount_finish >= 5000 then
			state = stateBib.FINISHED_2;
			SendGaz4_Rank(rank);
		end
	elseif state == stateBib.FINISHED_2 then
		local tickCount = app.GetTickCount();
		if tickCount - tickcount_finish >= 10000 then
			tickcount_finish = 0;
			state = stateBib.RUNNING;
		end
	end
...
function SendGaz4_Rank(rk)
	...
	local packet = 
		'A'.. 											-- Identifier Board
		string.format('%3d', tonumber(rk)); 			-- RK 
