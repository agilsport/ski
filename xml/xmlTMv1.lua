...
function afterFisresults(t)
	...
	if mode == 'import' then
		if #xmlError == 0 then
			...
			if tEvenement:IsCellNull('Code_activite', 0) then tEvenement:SetCell('Code_activite', 0, 'TM') end
			...
			if codeEntite == 'FIS' then
				if tEpreuve:IsCellNull('Code_grille_categorie', 0) then tEpreuve:SetCell('Code_grille_categorie', 0, 'FIS-TM') end
				if tEpreuve:IsCellNull('Code_categorie', 0) then tEpreuve:SetCell('Code_categorie', 0, '*') end
			else
				if tEpreuve:IsCellNull('Code_grille_categorie', 0) then tEpreuve:SetCell('Code_grille_categorie', 0, 'FFS-TM') end
				if tEpreuve:IsCellNull('Code_categorie', 0) then tEpreuve:SetCell('Code_categorie', 0, '*') end
			end
			...
			-- 2 Pénalités : Gate et Jump
			for i=0,tEpreuve_Alpine_Manche:GetNbRows()-1 do
				tEpreuve_Alpine_Manche:SetCell("Nb_penalite", i, 2);
				tEpreuve_Alpine_Manche:SetCell("Val_penalite", i, 0);
			end
...
function beforeRaceheader(t)
	if mode == 'import' then
		...
		if t.attributes.Sector ~= nil then
			tEvenement:SetCell('Code_activite', 0, t.attributes.Sector);
		end
	end
end
...
function contentFisCode(t)
	if mode == 'import' then 
		...
		code_entite = tEvenement:GetCell('Code_entite', 0);
		-- Message('code_entite'..code_entite);
		-- Message('code_activite'..code_activite);
		--Message('t.content'..t.content)
		if code_entite == 'FIS' then
			if code_activite == 'TM' then
				return 'ITM'..t.content;
			elseif code_activite == 'BIATH' then
				return 'IBU'..t.content;	
			elseif code_activite == 'FS' then
				return 'IFS'..t.content;	
			elseif code_activite == 'SB' then
				return 'ISB'..t.content;	
			elseif code_activite == 'FOND' then
				return 'IFO'..t.content;	
			elseif code_activite == 'CN' then
				return 'ICN'..t.content;
			elseif code_activite == 'SAUT' then
				return 'ISA'..t.content;
			elseif code_activite == 'SS' then
				return 'ISV'..t.content;
			else
...
function afterResultdetail_classified(tSrc)
	if mode == "import" then
		local t = tSrc;
		if t.attributes.Run ~= nil then
			local run = tonumber(t.attributes.Run) or 0;
			if run >= 1 and run <= 2 then
				local pena1 = 0;
				if t.children.gatepenaltytime ~= nil and type(t.children.gatepenaltytime[1].content) == 'string' then
					if string.len(t.children.gatepenaltytime[1].content) > 0 then
						pena1 = math.floor(tonumber(t.children.gatepenaltytime[1].content));	
					end
				end

				local pena2 = 0;
				if t.children.jumppenaltytime ~= nil and type(t.children.jumppenaltytime[1].content) == 'string' then
					if string.len(t.children.jumppenaltytime[1].content) > 0 then
						pena2 = math.floor(tonumber(t.children.jumppenaltytime[1].content));	
					end
				end
				
				local pena = string.format('%#02d',pena1)..'/'..string.format('%#02d',pena2);
				tResultat_Manche:SetCell('Penalite', run-1, pena);
			end
		end
	end
end
...
Raceheader = { 
	...
	children = {
		...
		{ name = 'Discipline', content = { 'CL','SP','GS','PSL','SP2-QL','SP2','TPS','PS' }, field = 'Epreuve.Code_discipline' },
...
TM_resultdetail_classified = {
	required = '0-2',
	attributes = {  {name = 'Run', type_value = 'integer'} },
	
	children = {
		{ name = 'gatepenaltytime', required = '0-1' },
		{ name = 'jumppenaltytime', required = '0-1' }
	},
	after = afterResultdetail_classified
};
...
TM_race = {
	children = { 
		...
		{ name = 'TM_classified', children = {
				{ name = 'TM_ranked', required = '0-99999', before = beforeTM_ranked,  attributes = {{name ='Status', value = 'QLF'}}, children = {
						...
						{ name = 'TM_result', props = TM_result_classified },
						{ name = 'TM_resultdetail', props = TM_resultdetail_classified },
					}, after = afterTM_ranked 
				}
			}
		},
		{ name = 'TM_notclassified', children = {
				{ name = 'TM_notranked', required = '0-99999', before = beforeTM_notranked,
